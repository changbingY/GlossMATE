...
output_root = Path("Qwen2.5-7B-Sigmorphon_result_abb_explain")
gloss_dict = {}
...
def translate_gloss_file_vllm(llm, sampling_params, input_path: Path, output_path: Path, lang: str):
    ...
    for idx, block in enumerate(blocks):
        ...
        if gloss_line:
            # Build gloss explanation
            explanation_lines = []
            gloss_tokens = gloss_line.replace('-', ' ').split()
            for token in gloss_tokens:
                if token.isupper() and token in gloss_dict:
                    explanation_lines.append(f'- "{token}" indicates {gloss_dict[token]}.')
            ...
            prompt = (
                ...
                f"{explanation_block}"
                "Please return only the English translation of the sentence. Do not say anything else."
            )
    ...
    if not prompts:
        print(f"No gloss lines found in {input_path.name}")
        return
